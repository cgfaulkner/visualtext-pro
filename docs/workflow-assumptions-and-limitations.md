...
## 0. Output and batch layout

**Disclosure slide:** Every PPTX save path calls `ensure_disclosure_slide(presentation)` (shared/disclosure_slide.py) before saving. Processed decks are assumed to have a standard "Disclosures" slide at position 2 when not already present elsewhere. Regression tests: `tests/test_disclosure_slide.py` (empty deck, one-slide, insert at index 1, no duplicate, no insert when elsewhere, layout, offline E2E).

**Staged batch:** When processing a directory/glob, manifest and per-file artifacts are under the input directory at `<input_root>/staged_runs/<run_id>/` (config: `staged_batch.staging_root`). Artifacts live under `run_dir/outputs/<relative_path>/`.
...
```
**Limitation**: When the connectivity manager is **not** used, there is no retry logic, circuit breaker, or health checks—single-attempt failures. When the connectivity manager **is** used, preflight health checks and circuit breaker behavior apply as documented in p5 (Decisions 6.2, 6.3) and p6 (Ollama HTTP API / Connection Management).
