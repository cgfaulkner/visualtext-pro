...
│   ├── system-overview.md     # Single-page pipeline overview
│   ├── artifacts_thumbs_vs_normalized.md
│   ├── batch_completion_criteria.md
│   ├── batch_manifest_skip_explained.md
│   ├── batch_resume_smoke_test.md
│   ├── entry-points-and-call-flow.md
│   ├── execution-path-trace.md
│   ├── slide-processing.md
│   ├── image-processing-flow.md
│   ├── alt-text-generation-decisions.md
...
- **Access Method**: Local inference via Ollama HTTP API (no CLI invocation)
...
- **Connectivity**: `shared/llava_connectivity.py::LLaVAConnectivityManager` (preflight health checks, circuit breaker, configurable unavailable modes)
- **Batch offline behavior**: When provider is offline, batch `process` can abort (exit 2) or inject placeholders; see README "Provider Availability & Offline Modes"
...
- Architecture docs use descriptive filenames (entry-points-and-call-flow, execution-path-trace, etc.).
