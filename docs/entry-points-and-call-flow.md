...
**CLI Commands**:
- `python altgen.py process <path>` → Processes single file or batch (directory/glob)
- `python altgen.py analyze <path>` → Generates review document only
...
main()
  ...
  │     │     ├─> resolve_run_folder_and_manifest() (shared/run_folder.py)
  │     │     │     → run_dir = input_root/staged_runs/<run_id>/, manifest_path = run_dir/manifest.json
  │     │     │     Flags: --run-id, --resume-manifest, --force
  │     │     └─> PPTXBatchProcessor.process_batch(files, manifest_path, input_root, run_dir)
  │     │           └─> _process_single() → subprocess with artifact_base = run_dir/outputs/<rel>/
  │     │
...
**Batch (staged runs):** When `process` is given a directory or glob, the run folder and manifest are under the **input directory** at `staged_runs/<run_id>/` (config: `staged_batch.staging_root`). Per-file artifacts go under `run_dir/outputs/<relative_path>/`. Resume with `--run-id <id>` or `--resume-manifest <path>`; `--force` skips manifest read/write.
