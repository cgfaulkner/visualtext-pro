...
**Location**: Local inference via Ollama HTTP API  
**Usage**: Primary AI model for generating descriptive ALT text from images

VisualText Pro integrates with Ollama only via its HTTP API; it does not invoke the `ollama` CLI.
...
- **Preflight health checks**: Before batch processing, connectivity and model availability are validated; if the provider is offline and `--offline-mode=abort`, the batch exits with code 2 and no files are processed.
- **Circuit breaker**: Repeated failures can open the circuit so further requests are not sent until recovery; behavior is configurable via `ai_providers.providers.llava.resilience.circuit_breaker`.
- **Configurable unavailable modes**: When the provider is unavailable (e.g. preflight failure or circuit open), behavior can be set to `fail_fast`, `skip_generation`, or `defer_generation` in config.
...
**Offline placeholder behavior** (batch `process` only): When the provider is offline and `--offline-mode` is `fill-missing` or `overwrite-all`, the CLI can inject placeholder ALT text without calling the API; `--placeholder-scope` controls whether only images or all visuals are targeted. See README section "Provider Availability & Offline Modes".
