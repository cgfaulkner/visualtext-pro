...
21. **`core/pptx_alt_injector.py::PPTXAltTextInjector.inject_alt_text_from_mapping()`** (continued)
    - Calls `ensure_disclosure_slide(presentation)` (shared/disclosure_slide.py) to insert standard "Disclosures" slide at position 2 when not already present
...
**Preflight / circuit breaker:** Batch `process` runs a provider preflight before processing; if the provider is offline and `--offline-mode=abort`, the run exits with code 2. Per-request generation is also gated by preflight and circuit breaker (see p5, p6).
