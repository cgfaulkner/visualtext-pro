...
ALT text generation is subject to **preflight health checks** (provider and model availability), **circuit breaker** behavior after repeated failures, and **configurable unavailable modes** (`fail_fast`, `skip_generation`, `defer_generation` in `ai_providers.providers.llava.resilience`). When the provider is offline, batch `process` can either abort (exit 2) or inject placeholders; see README "Provider Availability & Offline Modes" and [external-dependencies.md](external-dependencies.md).

**LLaVA image path:** At the LLaVA entrypoint, image paths are validated. Thumbnail paths are either **converted** to normalized images (default: `paths.strict_llava_gate: false`) or **rejected** (strict mode). The provider is never given a thumbnail path; only normalized paths are used. See [artifacts_thumbs_vs_normalized.md](artifacts_thumbs_vs_normalized.md).
