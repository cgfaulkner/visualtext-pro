...
**Image derivatives by purpose:** **Thumbnails** are used only for review/DOCX display (e.g. small previews). **Normalized images** (crops, converted temp images) are used only as LLaVA/model input. The LLaVA entrypoint validates paths and, by default, converts thumbnail paths to normalized images before calling the provider. See [artifacts_thumbs_vs_normalized.md](artifacts_thumbs_vs_normalized.md).
...
Generation is also gated by **circuit breaker** state and **configurable unavailable modes** (e.g. `fail_fast`, `skip_generation`, `defer_generation`); see p5 and p6.
