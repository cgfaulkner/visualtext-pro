...
- `FlexibleAltGenerator.generate_alt_text()` → LLaVA API calls (subject to preflight health checks, circuit breaker, and configurable unavailable modes; see [alt-text-generation-decisions.md](alt-text-generation-decisions.md) and [external-dependencies.md](external-dependencies.md)). Only **normalized** image paths are sent to LLaVA; thumbnails are for review/DOCX only (see [artifacts_thumbs_vs_normalized.md](artifacts_thumbs_vs_normalized.md)).
- `PPTXAltTextInjector.inject_alt_text_from_mapping()` → XML injection (after `ensure_disclosure_slide()`).

For **batch** `process` via `altgen.py`, when the provider is offline the run may **abort** (exit 2) or inject **placeholder ALT** only, depending on `--offline-mode` and `--placeholder-scope`; see README "Provider Availability & Offline Modes". Batch manifest and per-file artifacts live under the input directory at `staged_runs/<run_id>/` (see [batch_completion_criteria.md](batch_completion_criteria.md)).
...
4. Creates artifact directories (`RunArtifacts.create_for_run()` - session-specific `.alt_pipeline_{id}/` or, when batch, under `run_dir/outputs/<relative_path>/` from `VISUALTEXT_ARTIFACT_BASE_DIR`)
5. Enters smart recovery context (`smart_recovery_context()` - error recovery strategies)

**Outputs:**
- Lock file: `.{filename}.lock`
- Artifact directory: `.alt_pipeline_{session_id}/` or `run_dir/outputs/<rel>/` when batch (staged runs)
...
5. Ensures disclosure slide: `ensure_disclosure_slide(presentation)` (shared/disclosure_slide.py) — inserts standard "Disclosures" slide at position 2 when not already present (indicates AI-generated ALT text).
6. Saves presentation: `presentation.save(output_path)`

**Outputs:**
- Modified PPTX file with ALT text in XML structure and (when needed) disclosure slide at position 2
