...
**Post-injection (before save):** The pipeline calls `ensure_disclosure_slide(presentation)` so that every processed PPTX has a standard "Disclosures" slide at **position 2** (second slide) when not already present, indicating AI was used to create ALT text.
